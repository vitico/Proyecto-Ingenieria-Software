...
class Cell extends React.Component<CellProps> {
    ...
    getHeight() {
        ...
        return typeof height === 'function' ? height(rowData as any) : height;
    }
    ...
    renderTreeNodeExpandIcon() {
        ...
        const expandButton = (
            <ExpandIconComponent className={this.addPrefix('expand-icon')} />
        );
        if (this.isTreeCol() && hasChildren && rowData?.children?.length > 0) {
            return (
                <span
                    role="button"
                    tabIndex={-1}
    ...
    render() {
        ...
        const contentStyles: React.CSSProperties = {
            ...
            height: nextHeight,
            textAlign: align,
            [rtl ? 'paddingRight' : 'paddingLeft']: this.isTreeCol()
                ? (depth as number) * LAYER_WIDTH + 10
                : null,
            ...style,
        };
        ...
        let cellContent =
            isNullOrUndefined(children) && rowData
                ? _.get(rowData, dataKey as string)
                : children;
