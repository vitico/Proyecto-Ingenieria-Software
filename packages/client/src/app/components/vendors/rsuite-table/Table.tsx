...
export class Table extends React.Component<TableProps, TableState> {
    ...
    @observable data = ([] as any) as IObservableArray<any>;

    translateDOMPositionXY: any = null;
    scrollListener: any = null;

    tableRef = React.createRef<any>();
    scrollbarYRef = React.createRef<any>();
    scrollbarXRef = React.createRef<any>();
    tableBodyRef = React.createRef<any>();
    affixHeaderWrapperRef = React.createRef<any>();
    mouseAreaRef = React.createRef<any>();
    headerWrapperRef = React.createRef<any>();
    wheelWrapperRef = React.createRef<any>();
    tableHeaderRef = React.createRef<any>();

    tableRows: { [key: string]: [HTMLElement, any] } = {};
    mounted = false;
    disableEventsTimeoutId: any = null;
    scrollY = 0;
    scrollX = 0;
    wheelHandler: any;
    minScrollY: any;
    ...
    componentDidMount() {
        ...
        this.dispose2 = reaction(
            ...
            (data) => {
                console.log('should calculate', data);
                this.calculateRowMaxHeight();
                this.props.onDataUpdated?.(data, this.scrollTo);

                const maxHeight =
                    this.props.data.length *
                    (typeof this.props.rowHeight! === 'function'
                        ? this.props.rowHeight?.((null as unknown) as object)
                        : this.props.rowHeight!);
                // 当开启允许更新滚动条，或者滚动条位置大于表格的最大高度，则初始滚动条位置
                if (
                    this.props.shouldUpdateScroll ||
                    Math.abs(this.scrollY) > maxHeight
                ) {
                    this.scrollTo({ x: 0, y: 0 });
                }
                this.calculateTableContextHeight({} as any);
            }
        );
        ...
        bindElementResize(
            ...
            debounce(this.calculateTableWidth, 400)
        );

        const options = { passive: false };
        const tableBody = this.tableBodyRef.current;
        if (tableBody) {
            this.wheelListener = on(
                tableBody,
                'wheel',
                this.wheelHandler.onWheel,
                options
            );
            this.touchStartListener = on(
                tableBody,
                'touchstart',
                this.handleTouchStart,
                options
            );
            this.touchMoveListener = on(
                tableBody,
                'touchmove',
    ...
    shouldComponentUpdate(nextProps: TableProps, nextState: TableState) {
        const _cacheChildrenSize = flatten((nextProps.children as any[]) || [])
            .length;
        ...
        if (!_.isEqual(data, this.props.data)) {
            ...
            const maxHeight =
                ...
                (typeof rowHeight! === 'function'
                    ? rowHeight?.((null as unknown) as object)
                    : rowHeight!);
            // 当开启允许更新滚动条，或者滚动条位置大于表格的最大高度，则初始滚动条位置
            if (
                this.props.shouldUpdateScroll ||
    ...
    getExpandedRowKeys() {
        ...
        return typeof expandedRowKeys === 'undefined'
    ...
    getScrollCellGroups() {
        return this.tableRef.current?.querySelectorAll(
    ...
    getTableHeaderHeight(): number {
        ...
        return showHeader ? (headerHeight as number) : 0;
    }
    ...
    getTableHeight() {
        ...
        return autoHeight
            ? Math.max(headerHeight + contentHeight, minHeight as number)
            : height;
    }
    ...
    getCellDescriptor() {
        ...
        const headerCells: any[] = []; // Table header cell
        const bodyCells: any[] = []; // Table body cell
        ...
        React.Children.forEach(columns, (column, index) => {
            if (React.isValidElement(column)) {
                ...
                let nextWidth =
                    this.state[
                        `${columnChildren[1].props.dataKey}_${index}_width`
                    ] ||
                    width ||
                    0;
    ...
    affixHorizontalScrollbar = () => {
        ...
        const headerHeight = this.getTableHeaderHeight();
        const bottom =
            typeof affixHorizontalScrollbar === 'number'
                ? affixHorizontalScrollbar
                : 0;
    ...
    affixTableHeader = () => {
        ...
        const fixedHeader =
            //@ts-ignore
            scrollY - (headerOffset.top - top) >= 0 &&
            scrollY < headerOffset!.top - top + contentHeight;

        if (this.affixHeaderWrapperRef.current) {
            toggleClass(
                this.affixHeaderWrapperRef.current,
                'fixed',
                fixedHeader
            );
        }
    ...
    handleWheel = (deltaX: number, deltaY: number) => {
        ...
        if (virtualized) {
            ...
            if (this.disableEventsTimeoutId) {
                cancelAnimationTimeout(
                    (this
                        .disableEventsTimeoutId as unknown) as KeyframeAnimationOptions
                );
            }
    ...
    handleTouchMove = (event: React.TouchEvent) => {
        ...
        if (event.touches) {
            ...
            if (
                !this.shouldHandleWheelY(deltaY) &&
                !this.shouldHandleWheelX(deltaX)
            ) {
                return;
            }
    ...
    updatePositionByFixedCell() {
        ...
        const showRightShadow =
            width - contentWidth - SCROLLBAR_WIDTH !== this.scrollX;
    ...
    shouldRenderExpandedRow(rowData: object) {
        ...
        let result =
            ...
            !isTree &&
            expandedRowKeys.some((key) => key === getRowKey(rowData, rowKey));
    ...
    calculateRowMaxHeight() {
        ...
        if (wordWrap) {
            const tableRowsMaxHeight: number[] = [];
    ...
    calculateTableContextHeight(prevProps?: TableProps) {
        ...
        if (
            Math.abs(this.scrollY) + height - headerHeight >
    ...
    getControlledScrollTopValue(value) {
        ...
        value = Math.min(
            value,
            Math.max(0, contentHeight - (height - headerHeight))
        );
    ...
    scrollTop = (top = 0) => {
        ...
        if (
            ...
            this.state.contentHeight > this.props.height
        ) {
            this.forceUpdate();
        }
    };
    ...
    // public method
    scrollLeft = (left = 0) => {
        const [scrollX, handleScrollX] = this.getControlledScrollLeftValue(
            left
        );
    ...
    bindTableRowsRef = (index: number | string, rowData: any) => (
        ref: HTMLElement
    ) => {
        if (ref) {
    ...
    renderRowData(
        ...
        let curRowKey = getRowKey(rowData, rowKey);
        const nextRowKey =
            typeof curRowKey !== 'undefined' ? curRowKey : props.key;
        ...
        const expandedRowKeys = this.getExpandedRowKeys() || [];
        const expanded = expandedRowKeys.some((key) => key === curRowKey);
        const cells: any[] = [];
    ...
    renderTableBody(bodyCells: any[], rowWidth: number) {
        ...
        if (data) {
            ...
            const maxTop = minTop + height + (rowExpandedHeight as number);
       ...
       */
            if ((isUncertainHeight && virtualized) || !virtualized) {
                for (let index = 0; index < data.length; index++) {
                    ...
                    const shouldRenderExpandedRow = this.shouldRenderExpandedRow(
                        rowData
                    );
                    ...
                    if (typeof rowHeight === 'function') {
                        ...
                        if (shouldRenderExpandedRow) {
                            nextRowHeight += rowExpandedHeight as number;
                        }
                    }
                    ...
                    if (isTree) {
                        ...
                        if (
                            !shouldShowRowByExpanded(expandedRowKeys, parents)
                        ) {
                            continue;
                        }
                    }
         ...
         */
                ...
                const startIndex = Math.max(
                    Math.floor(minTop / nextRowHeight),
                    0
                );
                const endIndex = Math.min(
                    startIndex + Math.ceil(bodyHeight / nextRowHeight),
                    data.length
                );
                ...
                for (let index = startIndex; index < endIndex; index++) {
                    ...
                    this._visibleRows.push(
                        this.renderRowData(bodyCells, rowData, rowProps, false)
                    );
                }
            }
        }
    ...
    renderInfo() {
        ...
        const emptyMessage = (
            <div className={this.addPrefix('body-info')}>
                {locale?.emptyMessage as any}
            </div>
        );
        ...
        const loadingElement = (
            <div className={this.addPrefix('loader-wrapper')}>
                <div className={this.addPrefix('loader')}>
                    ...
                    <span className={this.addPrefix('loader-text')}>
                        {locale?.loading}
                    </span>
                </div>
            </div>
                ...
                <Observer>
                    {() => (
                        ...
                        >
                            ...
                            {children &&
                                this.renderTableBody(bodyCells, rowWidth)}
